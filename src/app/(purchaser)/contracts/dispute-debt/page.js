...
const NEED_OPTIONS = [
  {
    value: NEED_LETTER,
    title:
      "A legal letter addressed to a debtor demanding payment of an outstanding debt (including revisions based on client feedback)",
    pricing: "Lump sum fixed price",
    note: "Any offers you receive will be for a lump sum fixed price.",
    tooltip:
      "Any offers you receive will include the preparation of the legal letter, necessary revisions on the basis of your feedback to the legal service provider and sending of the legal letter to the debtor.",
    icon: "template",
  },
  {
    value: NEED_OCCASIONAL,
    title:
      "Occasional support with the debt collection process (including, for example, legal review of communications from the debtor or legal advice regarding next steps during different stages of the collection process, if requested)",
...
const PAGE_PATH = "/contracts/dispute-debt";
const REQUEST_DRAFT_TYPE = "debtCollection";
const DRAFT_EMPTY_TEXT = "No saved Debt Collection drafts found.";

function isLetterNeed(need) {
  return String(need || "").startsWith("A legal letter addressed to a debtor");
}
...
export default function DisputeDebt() {
  ...
  const isDebtLetter = isLetterNeed(formData.need);
  const isFixedFee = isDebtLetter;
  ...
  const validateStep = (step) => {
    ...
    if (step === 1) {
      if (!String(formData.confidential || "").trim()) {
        return {
          error: "Please provide the name of the debtor.",
          field: "confidential",
        };
      }
      return null;
    ...
    if (step === 2) {
      if (!formData.description) {
        return {
          error: "Please describe briefly the debt you are collecting.",
          field: "description",
        };
      }
      return null;
    ...
    if (type === "checkbox") {
      ...
      if (name === "providerSource") {
        ...
        const paymentTypeChanged =
          name === "need" &&
          isLetterNeed(value) !== isLetterNeed(formData.need);
        setFormData({
          ...formData,
          [name]: value,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        requestSubcategory: "Debt Collection Support",
        scopeOfWork: formData.need,
        description: formData.description || "",
        additionalBackgroundInfo: formData.background || "",
        backgroundInfoFiles: [],
  ...
  return (
    <>
      <form ref={formRef} onSubmit={handleSubmit} noValidate>
        <RequestWizard
          categoryLabel="Help with Debt Collection"
          steps={WIZARD_STEPS}
            ...
            <div className="space-y-6">
              <div data-field="confidential">
                <h4 className="mb-2 text-sm font-semibold text-gray-900">
                  ...
                  domicile of the debtor from whom you are collecting the debt.
                  If you do not want your name and the name of the debtor to be
          ...
          {currentStep === 2 ? (
            <div className="space-y-6">
              <div data-field="description">
                <h4 className="mb-2 text-sm font-semibold text-gray-900">
                  Please describe briefly the debt you are collecting (What is
                  the amount of the receivable and on what grounds are you
                  collecting it? What actions have you taken so far to collect
                  the receivable? On what grounds is the debtor refusing to pay
                  the receivable?)
      ...
      >
        ...
        <Section title="Contract Price (Lump Sum Fixed Fee or Blended Hourly Rate) and Currency">
          {formData.need.includes(NEED_LETTER) ? (
            <>
              {`Lump Sum Fixed Fee ${
            ...
            <>
              ...
              <p className="text-md mt-2">
                ...
                will be valid until the relevant outstanding debt(s) has been
                collected or the collection of the relevant outstanding debt(s)
                has been abandoned, whichever comes first.
              </p>
            </>
          )}
          <p className="text-md mt-2">
        ...
        <Section title="Description of the Debt Being Collected ">
