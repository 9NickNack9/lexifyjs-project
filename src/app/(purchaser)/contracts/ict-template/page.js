...
const PAGE_PATH = "/contracts/ict-template";
...
export default function IctTemplate() {
  ...
  const validateStep = (step) => {
    if (step === 0) {
      if (templateboxes.length === 0) {
        return {
          error: "Please choose at least one template type.",
          field: "templateboxes",
        };
      }
  ...
  useEffect(() => {
    window.__LEXIFY_REQUEST_CONTEXT__ = {
      requestType: "ICT/IT Contract Template",
      legalArea: [
  ...
  return (
    <>
      <form ref={formRef} onSubmit={handleSubmit} noValidate>
        <RequestWizard
          categoryLabel="Help with Preparing an ICT/IT Contract Template"
          steps={WIZARD_STEPS}
      ...
      >
        ...
        <Section title="Scope of Work">
          <p>
            Preparation of the following ICT/IT related contract and/or document
            template(s) for the Client:
          </p>
          {templateboxes.length > 0
            ...
            : "-"}
          <p className="pt-4">
            The work includes the preparation of the listed document(s) and
            necessary revisions on the basis of the Client&apos;s feedback to
