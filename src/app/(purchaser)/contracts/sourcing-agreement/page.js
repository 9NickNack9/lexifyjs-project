...
const LUMP_SUM_TEMPLATE_NOTE =
  "Any offers you receive will be for a lump sum fixed price and include the preparation of the document(s) and necessary revisions on the basis of your feedback to the legal service provider.";
...
const NEED_OPTIONS = [
  {
    ...
    note: LUMP_SUM_TEMPLATE_NOTE,
    icon: "template",
  },
  {
    value: NEED_CUSTOM,
    title: "Customized for the buying of a specific product or service type",
    pricing: "Lump sum fixed price",
    note: LUMP_SUM_TEMPLATE_NOTE,
    icon: "review",
  },
...
const WIZARD_STEPS = [
  {
    ...
    title: "Do you want your new sourcing agreement template to be:",
  },
...
const REQUEST_DRAFT_TYPE = "sourcingAgreement";
const DRAFT_EMPTY_TEXT = "No saved Sourcing Agreement Template drafts found.";
...
export default function SourcingAgreement() {
  ...
  const isCustomized = formData.templateType === NEED_CUSTOM;
  const selectedNeed = NEED_OPTIONS.find(
    (option) => option.value === formData.templateType,
  );
  ...
  const validateStep = (step) => {
    ...
    if (id === "need") {
      if (!formData.templateType) {
        return { error: "Please select a template type.", field: "need" };
      }
      return null;
    }
    if (id === "background") {
      if (isCustomized && !formData.description) {
        return {
          error:
            "Please describe the product/service type for the customized template.",
          field: "description",
        };
      }
      return null;
  ...
  const drafts = useRequestDrafts({
    ...
    pagePath: "/contracts/sourcing-agreement",
    formData,
    setFormData,
  ...
  useEffect(() => {
    window.__LEXIFY_REQUEST_CONTEXT__ = {
      requestType: "Sourcing Agreement Template",
      scopeOfWork: formData.templateType,
  ...
  return (
    <>
      <form ref={wizard.formRef} onSubmit={handleSubmit} noValidate>
        <RequestWizard
          categoryLabel="Help with Preparing a Sourcing Agreement Template"
          steps={WIZARD_STEPS}
              ...
              {isCustomized ? (
                <div data-field="description">
                  <h4 className="mb-2 text-sm font-semibold text-gray-900">
                    Please describe the product or service type for which the
                    sourcing agreement template should be customized{" "}
          ...
          {WIZARD_STEPS[wizard.currentStep]?.id === "review" ? (
            <div className="space-y-6">
              <dl className="rounded-xl border border-gray-200 px-4">
                <SummaryRow
                  label="Sourcing agreement template needed"
      ...
      >
        ...
        <Section title="Scope of Work">
          Preparation of a sourcing agreement template for the Client.
          <br />
          The work includes the preparation of the agreement template
          documentation and necessary revisions on the basis of the
          Client&apos;s feedback to the Legal Service Provider.
        </Section>
